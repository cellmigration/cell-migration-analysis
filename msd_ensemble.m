function [alpha_condition, SSresid_condition, rsq_condition] = msd_ensemble(cond, conc, features)
...
ylabel('MSD')
xlabel('Tau')
title(['PC3 Cells, ', cond, ' ', num2str(conc), ' ug/ml Collagen', ' (> ',num2str(data_points_min), ' points)'])
...
ylabel('Ln(MSD)')
xlabel('Ln(Tau)')
title(['PC3 Cells, ', cond, ' ', num2str(conc), ' ug/ml Collagen'])
...
msd_figure_file_name = [figure_directory, '\','msd-',cond,'-', num2str(conc),'.png'];
print(msd_figure,'-dpng',msd_figure_file_name)
alpha_figure_file_name = [figure_directory, '\','alpha-',cond,'-', num2str(conc),'.png'];
